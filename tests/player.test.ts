...
function createPlayer() {
  ...
  const player = new LyricPlayer({ audio: audio as unknown as HTMLAudioElement, lyrics: LRC });
  return { audio, player };
}
...
describe('LyricPlayer', () => {
  ...
  it('respects a runtime offset', () => {
    ...
    audio.tick(1.5);
    // With +1s offset, "Line one" (originally at 1.0) now effectively starts at 2.0
    expect(player.getCurrentLine()).toBeNull();
    audio.tick(2.5);
    expect(player.getCurrentLine()?.text).toBe('Line one');
  });
